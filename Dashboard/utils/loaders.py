...
def prepare_product_data(df_missing_products):
    ...
    if 'category' in df_missing_products.columns:
        ...
        category_summary.columns = ['category', 'total_relatos', 'qtd_produtos', 'preco_medio', 'valor_total_perdido']
    ...
    return df_missing_products, category_summary if 'category' in df_missing_products.columns else None
...
def load_data_from_db():
    ...
    try:
        ...
        if 'drivers' in ['orders', 'drivers', 'customers', 'missing_items', 'products']:
        ...
        try:
            ...
            if 'delivery_hour' in df_orders.columns or 'hour' in df_orders.columns or 'created_at' in df_orders.columns:
                # Tentar extrair hora da coluna de data/hora
                hour_col = next((col for col in ['delivery_hour', 'hour', 'created_at'] if col in df_orders.columns), None)
                if hour_col:
                            ...
                            if sample_date:
                                if isinstance(sample_date, str):
                                    ...
                                    for date_format in formats_to_try:
                                        ...
                                        df_orders['hora'] = pd.to_datetime(df_orders[hour_col], format='%Y-%m-%d %H:%M:%S', errors='coerce').dt.hour
                    ...
                    df_fraud_time = df_orders.groupby('hora').agg({
                        'id': 'count',  # Assumindo que 'id' é a coluna de ID do pedido
                    }).reset_index()
        ...
        try:
            ...
            if 'region' in df_customers.columns or 'state' in df_customers.columns or 'location' in df_customers.columns:
                # Usar a coluna de região/estado
                region_col = next((col for col in ['region', 'state', 'location'] if col in df_customers.columns), None)
                if region_col:
                    df_fraud_region = df_customers.groupby(region_col).agg({
                        'id': 'count'  # Assumindo que 'id' é a coluna de ID do cliente
                    }).reset_index()
        ...
        try:
            if 'products' in ['orders', 'drivers', 'customers', 'missing_items', 'products'] and 'missing_items' in ['orders', 'drivers', 'customers', 'missing_items', 'products']:
                ...
                product_id_col = next((col for col in df_missing.columns if 'product' in col.lower() and 'id' in col.lower()), None)
                ...
                if product_id_col and product_id_col in df_missing.columns:
                    ...
                    merged_df = pd.merge(product_counts, df_products, left_on='product_id', right_on='id', how='left')
                    
                    # Verificar colunas para categoria e preço
                    category_col = next((col for col in merged_df.columns if any(term in col.lower() for term in 
                                      ['category', 'categoria', 'type', 'tipo'])), None)
                    name_col = next((col for col in merged_df.columns if any(term in col.lower() for term in 
                                   ['name', 'nome', 'title', 'titulo'])), None)
                    price_col = next((col for col in merged_df.columns if any(term in col.lower() for term in 
                                    ['price', 'preco', 'valor', 'value'])), None)
                    ...
                    if 'product_id' in df_missing_products.columns:
                        df_missing_products['id'] = df_missing_products['product_id']
        ...
        try:
            if 'orders' in ['orders', 'drivers', 'customers', 'missing_items', 'products']:
                df_orders = pd.read_sql("SELECT * FROM orders", conn)
                
                # Verificar se há data
                date_col = next((col for col in df_orders.columns if any(term in col.lower() for term in 
                               ['date', 'data', 'created_at', 'created', 'datetime'])), None)
        ...
        # Clientes suspeitos
        try:
            if 'customers' in ['orders', 'drivers', 'customers', 'missing_items', 'products']:
                df_customers = pd.read_sql("SELECT * FROM customers", conn)
                ...
                df_suspicious_customers['customer_id'] = df_suspicious_customers['id'] if 'id' in df_suspicious_customers.columns else [f"C{i:03d}" for i in range(1, suspicious_count+1)]
                df_suspicious_customers['customer_name'] = df_suspicious_customers['name'] if 'name' in df_suspicious_customers.columns else [f"Cliente {i}" for i in range(1, suspicious_count+1)]
                df_suspicious_customers['customer_age'] = df_suspicious_customers['age'] if 'age' in df_suspicious_customers.columns else np.random.randint(18, 70, suspicious_count)
...
def generate_mock_data(data_type):
    ...
    if data_type == 'drivers':
        ...
        df = pd.DataFrame(data)
        df['id'] = df['product_id']  # ✅ Agora seguro
        return df
...
def prepare_data_for_time_analysis(df_fraud_time):
    ...
    """
    # Garantir que temos as colunas necessárias
    if df_fraud_time is None or df_fraud_time.empty or 'hora' not in df_fraud_time.columns:
        return df_fraud_time
